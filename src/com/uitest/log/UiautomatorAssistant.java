...
public class UiautomatorAssistant extends UiAutomatorTestCase {
	...
	/* log地址 */
	public static String m_logpathString = "/mnt/sdcard/ahavelog/PerformanceLog.txt";

	/*
	 * 定义“通过哪种方式来获得uiselector”的int标识， 如果以后想添加别的方法（例如 通过description
	 * 来获取），则可以参考此形式进行扩充
	 */
	...
	/* 构造传入UiDevice对象 */
	public UiautomatorAssistant(UiDevice device) {
		mdevice = device;
	}
	...
	/*
	 * 封装出通用的点击方法，供上面的public函数调用 如果以后想添加别的方法（例如 通过description 来获取），则可以在switch中扩充
	 */
	private boolean ClickByInfo(int CLICK, String str) {
		...
		case CLICK_ID:
			...
			uo = ElementManager.getUiObjectByResourceIdMatches(str);
			
			break;
		case CLICK_TEXT:
			//uiselector = new UiSelector().text(str);
			uo = ElementManager.getUiObjectByText(str);
			break;
		default:
			return false;
		}
	...
	public void TakeScreen(String descrip) {
		...
		String tmpPicPath = "/mnt/sdcard/ahavelog/runtmp.jpg";
		String filesPath = "/mnt/sdcard/ahavelog/" + datestr + "_" + descrip + ".jpg";
		
		File tmpPic = new File(tmpPicPath);
		File files = new File(filesPath);
		
		UiAutomationLog("TakeScreen: " + datestr + "_" + descrip + ".jpg");
		
		
		mdevice.takeScreenshot(tmpPic);
		ImageManager.CompressToSdcard(tmpPicPath, filesPath);
		//ImageManager.drawTextBitmap(bitmap, text)
	}
...
//				+ calendar.get(Calendar.SECOND) + ":";
		...
		try {
			fwlog = new FileWriter(UserConfig.saveAppLogPath, true);
